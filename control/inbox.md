...
## ORDER 023 · 2026-07-21T19:57:20Z · status: done
priority: P1
do: Program-end closeout (coordinator-dispatched under the owner's program-end prompt, 2026-07-21). Deliverable: docs/PROJECT-CLOSEOUT.md — durable summary for a cold reader (owner + fresh session): 11 rounds / 5,940 configs / 0 promoted, holdout SPENT, promotion CLOSED, paper-0001 WATCH (first evaluable bar ~early Aug 2026), R9/R10 confluence duals null with mechanism, R11 cross-asset causal-regime null, new-data-source direction owner-gated. True-up: docs/current-state.md restamped with the closeout pointer + the note that the Friday paper-grading cron dies at program close — run `python3 scripts/grade_paper.py` manually from 2026-07-24. SEAT CLOSED. control/ remains retired; this ORDER is the append-only done-state record.
why: program-end handoff before the 2026-07-22T00:00Z read-only cutoff — the whole-program state (11 rounds / 5,940 configs / 0 promoted, holdout SPENT, promotion CLOSED, paper lane, continuation steps) needed a single durable artifact for the owner and a cold future session, plus a trued-up living ledger.
done-when: docs/PROJECT-CLOSEOUT.md + the docs/current-state.md true-up landed on main via claude/final-closeout + auto-merge-enabler, and this ORDER is recorded done.
provenance: coordinator dispatch, 2026-07-21. (Decide-and-flag: control/ is recorded as RETIRED at HEAD, but the ORDER 019–022 precedent lands coordinator ORDERs here as the append-only done-state record; provenance decides, landed with this line.)
