#!/usr/bin/env python3
"""Round 3 slice 10 — FULL TREND FAMILIES on the 6 new instruments × daily.
...
families. This slice completes the TREND-family coverage of that surface
with the four existing trend families (``donchian``, ``ema_crossover``,
``macd``, ``supertrend_flip``) over 12-variant sub-grids of their published
axes, declared in ``trading_lab.sweeps`` BEFORE this script ran: 12
variants × 4 families × 6 instruments = 288 registered configs. Program
burden prior to this lane: 2432 (Round-3 slices 1-8 cumulative per the
synthesis budget ledger); program cumulative after: 2720. NO new strategy
code and NO new parameter territory (subset relations pinned in
``tests/test_sweeps.py``); the donchian sub-grid is disjoint from the
slice-3 probe grid so no config is registered twice. Honesty note:
``supertrend_flip`` had only ever been run on BTC-USD (the P1 video lane) —
this is its first run on equities/ETFs. The instruments stay lane-local
...
Usage: python3 scripts/run_r3_trend_new_tickers.py
"""
...
SWEEP_DIR = config.EXPERIMENTS_DIR / "sweeps" / "r3-trend-new-tickers"

# Program burden prior to this lane: 2432 = 680 pre-Round-3 + 1752 Round-3
# slices 1-8 (docs/research-round-3-results.md budget ledger).
PROGRAM_PRIOR_CONFIGS = 2432
...
def main() -> None:
    ...
    grid_sizes = sweeps.r3_trend_new_tickers_variants_per_family()
    lane_configs = sweeps.r3_trend_new_tickers_total_configs()
    program_cumulative = PROGRAM_PRIOR_CONFIGS + lane_configs
    assert grid_sizes == {"donchian": 12, "ema_crossover": 12,
                          "macd": 12, "supertrend_flip": 12} \
        and lane_configs == 288, \
        "grid drifted from the declared Round-3 slice-10 lane (trading_lab.sweeps)"
    assert not (set(sweeps.R3_TREND_NEW_TICKERS_INSTRUMENTS)
                & set(config.UNIVERSE)), \
        "slice-10 instruments must stay disjoint from the frozen universe"
    print(f"grids: {grid_sizes} × {len(sweeps.R3_TREND_NEW_TICKERS_INSTRUMENTS)} "
          f"instruments = {lane_configs} registered configs "
          f"(program cumulative {program_cumulative})")
    verdicts = []

    for ticker in sweeps.R3_TREND_NEW_TICKERS_INSTRUMENTS:
        ...
        for family in sweeps.R3_TREND_NEW_TICKERS_FAMILIES:
            variants = sweeps.r3_trend_new_tickers_variants(family)
            ...
            run_path = ledger.run_and_record(
                ...
                notes=(f"Round 3 trend-new-tickers sweep (post-holdout "
                       f"DEV-ONLY, promotion closed — ORDER 012 night-run, "
                       f"slice 10, extends R3 after the slices-1-8 "
                       ...
                       f"experiments/sweeps/r3-trend-new-tickers/"
            ...
            sweep_record = {
                ...
                "instrument_status": ("slice-3 lane-local instrument (not in "
                                      ...
                                      "PR #83 cache reused"),
                "family_status": ("existing strategy, no new code; "
                                  "supertrend_flip previously BTC-USD-only "
                                  "(P1 video lane) — first equities/ETF run"
                                  if family == "supertrend_flip" else
                ...
                "family_variants_tried": (grid_sizes[family]
                                          * len(sweeps.R3_TREND_NEW_TICKERS_INSTRUMENTS)),
