#!/usr/bin/env python3
"""Round 6 slice R6-A — volume families × 15 daily tickers (lane r6-volume).

POST-HOLDOUT, DEV-ONLY (ORDER 014 item 2 night-run; pre-registered protocol:
``docs/research-round-6-plan.md`` § R6-A, merged BEFORE any Round-6 outcome
existed). Promotion is CLOSED: nothing this script produces can be an
out-of-sample claim; KEEPs are dev-candidates only.

Families: ``obv_trend`` (long while OBV sits above its own trailing SMA;
``price_confirm=False`` is the pure-OBV within-family control arm) and
``mfi_reversion`` (Money Flow Index oversold cross-up reversion) — the
program's first volume-based indicator class. Grid declared in
``trading_lab.sweeps`` BEFORE this script ran (PR #116): 12 variants per
family × 15 tickers = 360 registered configs. Program burden prior to this
slice: 4359 (docs/research-round-5-results.md closing tally); program
cumulative after: 4719.
...
Usage: python3 scripts/run_r6_volume_sweep.py
"""
...
# Program burden prior to this slice: 4359 registered configs
# (docs/research-round-5-results.md closing tally). R6-A is the first
# Round-6 slice to run, so its 360 land on top of 4359.
PROGRAM_PRIOR_CONFIGS = 4359
...
def run_lane(ticker: str, family: str, grid_sizes: dict, lane_configs: int,
    ...
    variants = sweeps.r6_volume_variants(family)
    ...
    run_path = ledger.run_and_record(
        ...
        notes=(f"Round 6 slice R6-A volume sweep (post-holdout DEV-ONLY, "
               f"promotion closed — ORDER 014 item 2 night-run; "
    ...
    sweep_record = {
        ...
        "order": "ORDER 014 item 2 night-run (control/inbox.md); "
                 "pre-registered docs/research-round-6-plan.md § R6-A",
        ...
        "family_variants_tried": (grid_sizes[family]
                                  * len(sweeps.R6_VOLUME_INSTRUMENTS)),
...
def main() -> None:
    ...
    grid_sizes = sweeps.r6_volume_variants_per_family()
    lane_configs = sweeps.r6_volume_total_configs()
    program_cumulative = PROGRAM_PRIOR_CONFIGS + lane_configs
    assert grid_sizes == {"obv_trend": 12, "mfi_reversion": 12} \
        and lane_configs == 360, \
        "grid drifted from the declared Round-6 R6-A slice (trading_lab.sweeps)"
    print(f"grids: {grid_sizes} × {len(sweeps.R6_VOLUME_INSTRUMENTS)} "
          f"instruments = {lane_configs} registered configs "
          f"(program cumulative {program_cumulative}); cap {CAP_SECONDS} s")

    lanes = [(t, f) for t in sweeps.R6_VOLUME_INSTRUMENTS
             for f in sweeps.R6_VOLUME_FAMILIES]
