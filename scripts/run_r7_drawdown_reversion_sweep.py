#!/usr/bin/env python3
"""Round 7 slice R7-A — drawdown_reversion × 15 daily tickers (lane r7-drawdown-reversion).

POST-HOLDOUT, DEV-ONLY (ORDER 018 owner overnight research GO; pre-registered
protocol: ``docs/research-round-7-plan.md`` § R7-A, merged BEFORE any Round-7
...
Family: ``drawdown_reversion`` (normalized depth-from-peak state with a
hysteresis exit) — the program's first drawdown-anatomy indicator class.
Grid declared in ``trading_lab.sweeps`` BEFORE this script ran: 12 variants ×
15 tickers = 180 registered configs. Program burden prior to this slice: 5055
(docs/research-round-6-results.md closing tally + interim); program cumulative
after this R7-A slice: 5235.
...
Usage: python3 scripts/run_r7_drawdown_reversion_sweep.py
"""
...
FAMILIES = ("drawdown_reversion",)

SWEEP_DIR = config.EXPERIMENTS_DIR / "sweeps" / SLICE

# Program burden prior to this slice: 5055 registered configs
# (docs/research-round-6-results.md closing tally). R7-A is the first
# Round-7 slice to run, so its 180 land on top of 5055.
PROGRAM_PRIOR_CONFIGS = 5055
...
def run_lane(ticker: str, family: str, grid_sizes: dict, lane_configs: int,
    ...
    variants = sweeps.r7_drawdown_variants(family)
    ...
    run_path = ledger.run_and_record(
        ...
        notes=(f"Round 7 slice R7-A drawdown_reversion sweep (post-holdout "
    ...
    sweep_record = {
        ...
        "order": "ORDER 018 owner overnight research GO (control/inbox.md); "
                 "pre-registered docs/research-round-7-plan.md § R7-A",
...
def main() -> None:
    ...
    grid_sizes = {f: len(sweeps.r7_drawdown_variants(f)) for f in FAMILIES}
    lane_configs = sum(grid_sizes.values()) * len(sweeps.R7_INSTRUMENTS)
    program_cumulative = PROGRAM_PRIOR_CONFIGS + lane_configs
    assert grid_sizes == {"drawdown_reversion": 12} \
        and lane_configs == 180, \
        "grid drifted from the declared Round-7 R7-A slice"
