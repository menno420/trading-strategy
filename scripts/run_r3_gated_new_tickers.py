#!/usr/bin/env python3
"""Round 3 slice 12 — R2/GATED FAMILIES on the 6 new instruments × daily.
...
TSLA, JPM, XOM, TLT; slice 10 (r3-trend-new-tickers, PR #90) completed the
TREND-family coverage of that surface and slice 11 (r3-meanrev-new-tickers,
PR #91) the MEAN-REVERSION coverage. This slice completes the remaining
single-instrument coverage with the three R2/gated families
(``keltner_breakout``, ``vol_filtered_trend``, ``macd_supertrend``) over
pre-declared sub-grids of their published axes, declared in
``trading_lab.sweeps`` BEFORE this script ran: (6 + 12 + 12) variants × 6
instruments = 180 registered configs. Program burden prior to this lane:
3080 (2432 through the slices-1-8 synthesis + 288 slice 10 + 360 slice 11);
program cumulative after: 3260. NO new strategy code and NO new parameter
territory (subset relations pinned in ``tests/test_sweeps.py``).

Honesty notes — all three families were previously NARROW-UNIVERSE:
``vol_filtered_trend`` had only ever run on the Round-2 §3a four (AAPL,
MSFT, NVDA, GLD); ``keltner_breakout`` only on the Round-2 §3b four
(BTC-USD, META, AMZN, SLV); ``macd_supertrend`` only on BTC-USD (the P1
video lane) — this is its first run on equities/ETFs. ``keltner_breakout``
runs at 6 variants, not 12: its FULL published R2 grid is only 6 points
and inventing new n/m values would open new parameter territory.
``vol_filtered_trend`` reuses its full 12-variant R2 grid VERBATIM,
including the ``vol_filter=False`` control arm (slice-2/4 card convention:
gated grids commit their neutral arm). The instruments stay lane-local
(``config.UNIVERSE`` frozen); caches are the committed slice-3 ones.
...
2. **A walk-forward evaluation** of the grid (train 1008 bars ≈ 4y, test
   ...
   (``trading_lab.promotion.grade_promotion``, Bonferroni K = variants
   this lane: 6 or 12) is recorded alongside — informational only,
   promotion closed.
...
Usage: python3 scripts/run_r3_gated_new_tickers.py
"""
...
SWEEP_DIR = config.EXPERIMENTS_DIR / "sweeps" / "r3-gated-new-tickers"

# Program burden prior to this lane: 3080 = 2432 through the Round-3
# slices-1-8 synthesis (docs/research-round-3-results.md budget ledger)
# + 288 slice 10 (r3-trend-new-tickers, PR #90) + 360 slice 11
# (r3-meanrev-new-tickers, PR #91).
PROGRAM_PRIOR_CONFIGS = 3080
...
FAMILY_STATUS = {
    "keltner_breakout": ("existing strategy, no new code; previously run "
                         "only on the Round-2 §3b four (BTC-USD, META, "
                         "AMZN, SLV) — first run on these instruments; "
                         "full published R2 grid reused VERBATIM (6 "
                         "variants — expanding to ~12 would open new "
                         "parameter territory)"),
    "vol_filtered_trend": ("existing strategy, no new code; previously run "
                           "only on the Round-2 §3a four (AAPL, MSFT, "
                           "NVDA, GLD) — first run on these instruments; "
                           "full published 12-variant R2 grid reused "
                           "VERBATIM incl. the vol_filter=False control "
                           "arm"),
    "macd_supertrend": ("existing strategy, no new code; previously "
                        "BTC-USD-only (P1 video lane) — first "
                        "equities/ETF run; 12-variant sub-grid of the "
                        "36-variant video grid, MACD triple frozen at "
                        "12/26/9"),
}
...
def main() -> None:
    ...
    grid_sizes = sweeps.r3_gated_new_tickers_variants_per_family()
    lane_configs = sweeps.r3_gated_new_tickers_total_configs()
    program_cumulative = PROGRAM_PRIOR_CONFIGS + lane_configs
    assert grid_sizes == {"keltner_breakout": 6,
                          "vol_filtered_trend": 12,
                          "macd_supertrend": 12} \
        and lane_configs == 180, \
        "grid drifted from the declared Round-3 slice-12 lane (trading_lab.sweeps)"
    assert not (set(sweeps.R3_GATED_NEW_TICKERS_INSTRUMENTS)
                & set(config.UNIVERSE)), \
        "slice-12 instruments must stay disjoint from the frozen universe"
    print(f"grids: {grid_sizes} × {len(sweeps.R3_GATED_NEW_TICKERS_INSTRUMENTS)} "
          f"instruments = {lane_configs} registered configs "
          f"(program cumulative {program_cumulative})")
    verdicts = []

    for ticker in sweeps.R3_GATED_NEW_TICKERS_INSTRUMENTS:
        ...
        for family in sweeps.R3_GATED_NEW_TICKERS_FAMILIES:
            variants = sweeps.r3_gated_new_tickers_variants(family)
            ...
            run_path = ledger.run_and_record(
                ...
                notes=(f"Round 3 gated-new-tickers sweep (post-holdout "
                       f"DEV-ONLY, promotion closed — ORDER 012 night-run, "
                       f"slice 12, extends R3 after the slices-1-8 "
                       ...
                       f"experiments/sweeps/r3-gated-new-tickers/"
            ...
            sweep_record = {
                ...
                "instrument_status": ("slice-3 lane-local instrument (not in "
                                      ...
                                      "PR #83 cache reused"),
                "family_status": FAMILY_STATUS[family],
                "timeframe": TIMEFRAME,
                ...
                "family_variants_tried": (grid_sizes[family]
                                          * len(sweeps.R3_GATED_NEW_TICKERS_INSTRUMENTS)),
