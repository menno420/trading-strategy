#!/usr/bin/env python3
"""Round 3 slice 7 — cross-sectional lane on the EXPANDED 14-instrument
basket (`xsec_momentum` + NEW `xsec_reversal`) × daily.
...
The lab's second PORTFOLIO-level lane, mirroring the Round-2 R3 lane
(scripts/run_r2_xsec_momentum.py) exactly in methodology: each config is
ONE fixed portfolio rule over the whole basket — rank on the aligned
common date index, hold top-k (momentum) / bottom-k (reversal)
equal-weight, rebalance on the family's frozen cadence (21 bars monthly /
5 bars weekly), long/flat, no leverage. Basket `XSEC-14`: the 14 daily
equity/ETF instruments (frozen 8-ticker universe minus BTC-USD, plus the
six slice-3 instruments SPY/QQQ/TSLA/JPM/XOM/TLT). Grids declared in
``trading_lab.sweeps`` BEFORE this script ran: 6 + 6 = 12 registered
configs (portfolio lane: configs are grid points, NOT × instruments).
Program burden prior to this lane: 2132 (1844 pre-slice-6 + 288
r3-breakout); program cumulative after: 2144.
...
Usage: python3 scripts/run_r3_xsec_expanded.py
"""
...
SWEEP_DIR = config.EXPERIMENTS_DIR / "sweeps" / "r3-xsec-expanded"
...
def main() -> None:
    ...
    grid_sizes = sweeps.r3_xsec_expanded_variants_per_family()
    lane_configs = sweeps.r3_xsec_expanded_total_configs()
    program_cumulative = PROGRAM_PRIOR_CONFIGS + lane_configs
    assert grid_sizes == {"xsec_momentum": 6,
                          "xsec_reversal": 6} and lane_configs == 12, \
        "grid drifted from the declared Round-3 slice-7 lane (trading_lab.sweeps)"
    print(f"grids: {grid_sizes} over "
          f"{len(sweeps.R3_XSEC_EXPANDED_INSTRUMENTS)} instruments = "
          f"{lane_configs} registered configs "
          f"(program cumulative {program_cumulative})")

    # Data rail + alignment (Round-2 R3 handling, reused): default
    ...
    frames = {}
    for t in sweeps.R3_XSEC_EXPANDED_INSTRUMENTS:
    ...
    verdict_lines = []
    for family in sweeps.R3_XSEC_EXPANDED_FAMILIES:
        variants = sweeps.r3_xsec_expanded_variants(family)
        weight_fn = PORTFOLIO_STRATEGIES[family]
        rebal = sweeps.R3_XSEC_EXPANDED_REBALANCE_EVERY[family]
        ...
        record = {
            ...
            "notes": (f"Round 3 slice 7 (post-holdout DEV-ONLY, promotion "
                      f"closed — ORDER 012 night-run): top full-dev-period "
                      ...
                      f"live in experiments/sweeps/r3-xsec-expanded/"
                      f"{family}__{BASKET}.json. Portfolio lane: instrument "
                      f"{BASKET} = equal-weight top/bottom-k basket over "
                      f"{len(sweeps.R3_XSEC_EXPANDED_INSTRUMENTS)} "
                      ...
                      f"Round-2 R3 short-history convention); win_rate is "
        ...
        sweep_record = {
            "schema_version": 1,
            "sweep": "r3-xsec-expanded",
            "created_utc": datetime.now(timezone.utc).isoformat(
                timespec="seconds"),
            "git_sha": ledger.git_sha(),
            "family": family,
            "instrument": BASKET,
            "instruments": list(sweeps.R3_XSEC_EXPANDED_INSTRUMENTS),
