#!/usr/bin/env python3
"""P1 known-indicator sweep — trend-following family × all 8 tickers × hourly.

Lane: trend-following (sma_crossover, ema_crossover, macd, donchian) on the
full universe, HOURLY bars, pre-holdout dev data only (committed cache,
2023-08-10 → 2025-01-08, ~2476 bars/ticker — cache-only, zero network
fetches), realistic costs on (engine defaults: 5 bps slippage + 1 bp
commission per side).
...
1. **Per-variant full-dev-period backtests** for every grid point in
   ...
   (``experiments/sweeps/p1-trend-hourly/<family>__<ticker>.json``).
...
2. **A walk-forward evaluation** of the family grid
   (``trading_lab.walkforward.walk_forward``). Decide-and-flag: the lab's
   1008/252/252 BAR convention is kept for cross-lane comparability — on
   hourly bars that means ~7.4-month train windows and 252-bar (~7.4-week)
   test windows; on ~2476 bars it yields 5 splits and a stitched OOS of
   ~1260 bars ≈ 9.3 months (~2024-03 → 2025-01, a single regime). The
   stitched out-of-sample result is the only number reported as a finding,
   benchmarked against buy-and-hold over the exact same stitched OOS period.
...
Holdout rail: the loader excludes bars >= HOLDOUT_START by default; this
script additionally ASSERTS data_end < HOLDOUT_START for every ticker before
any backtest runs.

Usage: python3 scripts/run_p1_trend_hourly_sweep.py
"""
...
SWEEP_DIR = config.EXPERIMENTS_DIR / "sweeps" / "p1-trend-hourly"
    ...
    for ticker in config.UNIVERSE:
        ...
        for family in sweeps.TREND_FOLLOWING_FAMILIES:
            ...
            run_path = ledger.run_and_record(
                ...
                notes=(f"P1 trend-hourly sweep: top full-dev-period variant "
                       f"of {len(variants)} tried (post-hoc selection — "
                       f"in-sample; the walk-forward OOS finding lives in "
                       f"experiments/sweeps/p1-trend-hourly/"
            ...
            print(f"{ticker} {family}: n_bars={len(ohlcv)} "
                  f"splits={len(wf['splits'])} oos_sharpe="
