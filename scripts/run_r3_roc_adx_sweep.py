#!/usr/bin/env python3
"""Round 3 slice 2 — ROC momentum + ADX-filtered SMA × all 8 tickers × daily.
...
Families: ``roc_momentum`` (time-series rate-of-change momentum with an
entry/exit hysteresis band) and ``adx_filtered_sma`` (SMA crossover gated by
a Wilder ADX trending-regime filter; ADX period frozen at 14) — long/flat.
Grid declared in ``trading_lab.sweeps`` BEFORE this script ran: 12 variants
per family × 8 tickers = 192 registered configs. Program burden prior to
this lane: 872 (680 pre-Round-3 + 192 r3-stoch-willr); program cumulative
after: 1064.
...
Usage: python3 scripts/run_r3_roc_adx_sweep.py
"""
...
SWEEP_DIR = config.EXPERIMENTS_DIR / "sweeps" / "r3-roc-adx"

# Program burden prior to this lane: 680 pre-Round-3 (590 pre-Round-2 + 78
# Round 2 + 12 MTF Bollinger grid) + 192 r3-stoch-willr (slice 1, PR #81).
PROGRAM_PRIOR_CONFIGS = 872
...
def main() -> None:
    ...
    grid_sizes = sweeps.r3_roc_adx_variants_per_family()
    lane_configs = sweeps.r3_roc_adx_total_configs()
    program_cumulative = PROGRAM_PRIOR_CONFIGS + lane_configs
    assert grid_sizes == {"roc_momentum": 12,
                          "adx_filtered_sma": 12} and lane_configs == 192, \
        "grid drifted from the declared Round-3 slice-2 lane (trading_lab.sweeps)"
    print(f"grids: {grid_sizes} × {len(sweeps.R3_ROC_ADX_INSTRUMENTS)} "
          f"instruments = {lane_configs} registered configs "
          f"(program cumulative {program_cumulative})")
    verdicts = []

    for ticker in sweeps.R3_ROC_ADX_INSTRUMENTS:
        ohlcv = load_ohlcv(ticker, TIMEFRAME)  # holdout excluded by default
        for family in sweeps.R3_ROC_ADX_FAMILIES:
            variants = sweeps.r3_roc_adx_variants(family)
            ...
            run_path = ledger.run_and_record(
                ...
                notes=(f"Round 3 roc-adx sweep (post-holdout DEV-ONLY, "
                       ...
                       f"r3-roc-adx/{family}__{ticker}.json)"),
            )
            ...
            sweep_record = {
                ...
                "family_variants_tried": (grid_sizes[family]
                                          * len(sweeps.R3_ROC_ADX_INSTRUMENTS)),
