#!/usr/bin/env python3
"""Round 3 slice 8 — TRIX momentum + Ichimoku cloud trend × 12-ticker
mixed set × daily.
...
Families: ``trix_momentum`` (triple-smoothed EMA rate-of-change; committed
semantic: long while TRIX is above its EMA signal line — the
``signal_period == 0`` arm replaces the signal line with the constant 0,
recovering the classic TRIX zero-line rule, and is the committed
within-family control) and ``ichimoku_trend`` (Ichimoku cloud trend: long
when close is above BOTH displaced senkou spans AND tenkan > kijun;
committed exit: flat on a full cloud break below OR tenkan < kijun,
hysteresis in between; spans displaced forward the standard 26 bars using
only past data — no lookahead) — long/flat. Instruments: the slice-4/6
12-ticker mixed set reused verbatim — the frozen 8-ticker universe PLUS
four of the slice-3 new instruments (SPY, QQQ, TSLA, TLT — lane-local,
reusing the slice-3 caches; ``config.UNIVERSE`` untouched). Grid declared
in ``trading_lab.sweeps`` BEFORE this script ran: 12 variants per family ×
12 tickers = 288 registered configs. Program burden prior to this lane:
2144 (680 pre-Round-3 + 192 r3-stoch-willr + 192 r3-roc-adx + 108
r3-new-tickers + 288 r3-aroon-cci + 384 r3-meanrev-hourly + 288
r3-breakout + 12 r3-xsec-expanded); program cumulative after: 2432.
...
Usage: python3 scripts/run_r3_trix_ichimoku_sweep.py
"""
...
SWEEP_DIR = config.EXPERIMENTS_DIR / "sweeps" / "r3-trix-ichimoku"
...
# r3-meanrev-hourly (slice 5, PR #85) + 288 r3-breakout (slice 6, PR #86)
# + 12 r3-xsec-expanded (slice 7, PR #87).
PROGRAM_PRIOR_CONFIGS = 2144
...
def main() -> None:
    ...
    grid_sizes = sweeps.r3_trix_ichimoku_variants_per_family()
    lane_configs = sweeps.r3_trix_ichimoku_total_configs()
    program_cumulative = PROGRAM_PRIOR_CONFIGS + lane_configs
    assert grid_sizes == {"trix_momentum": 12,
                          "ichimoku_trend": 12} and lane_configs == 288, \
        "grid drifted from the declared Round-3 slice-8 lane (trading_lab.sweeps)"
    print(f"grids: {grid_sizes} × {len(sweeps.R3_TRIX_ICHIMOKU_INSTRUMENTS)} "
          f"instruments = {lane_configs} registered configs "
          f"(program cumulative {program_cumulative})")
    verdicts = []

    for ticker in sweeps.R3_TRIX_ICHIMOKU_INSTRUMENTS:
        ...
        for family in sweeps.R3_TRIX_ICHIMOKU_FAMILIES:
            variants = sweeps.r3_trix_ichimoku_variants(family)
            ...
            run_path = ledger.run_and_record(
                ...
                notes=(f"Round 3 trix-ichimoku sweep (post-holdout DEV-ONLY, "
                       ...
                       f"r3-trix-ichimoku/{family}__{ticker}.json)"),
            )
            ...
            sweep_record = {
                ...
                "family_variants_tried": (grid_sizes[family]
                                          * len(sweeps.R3_TRIX_ICHIMOKU_INSTRUMENTS)),
