#!/usr/bin/env python3
"""Round 3 slice 4 — Aroon trend + CCI reversion × 12-ticker mixed set × daily.
...
Families: ``aroon_trend`` (Aroon oscillator trend-following with an
entry/exit hysteresis band; the zero-band arm — the classic
Aroon-Up/Aroon-Down cross — is the committed within-family control) and
``cci_reversion`` (Lambert CCI mean-reversion, oversold cross-up entry) —
long/flat. Instruments: the frozen 8-ticker universe PLUS four of the
slice-3 new instruments (SPY, QQQ, TSLA, TLT — lane-local, reusing the
slice-3 caches; ``config.UNIVERSE`` untouched). Grid declared in
``trading_lab.sweeps`` BEFORE this script ran: 12 variants per family × 12
tickers = 288 registered configs. Program burden prior to this lane: 1172
(680 pre-Round-3 + 192 r3-stoch-willr + 192 r3-roc-adx + 108
r3-new-tickers); program cumulative after: 1460.
...
Usage: python3 scripts/run_r3_aroon_cci_sweep.py
"""
...
SWEEP_DIR = config.EXPERIMENTS_DIR / "sweeps" / "r3-aroon-cci"
...
def main() -> None:
    ...
    grid_sizes = sweeps.r3_aroon_cci_variants_per_family()
    lane_configs = sweeps.r3_aroon_cci_total_configs()
    program_cumulative = PROGRAM_PRIOR_CONFIGS + lane_configs
    assert grid_sizes == {"aroon_trend": 12,
                          "cci_reversion": 12} and lane_configs == 288, \
        "grid drifted from the declared Round-3 slice-4 lane (trading_lab.sweeps)"
    print(f"grids: {grid_sizes} × {len(sweeps.R3_AROON_CCI_INSTRUMENTS)} "
          f"instruments = {lane_configs} registered configs "
          f"(program cumulative {program_cumulative})")
    verdicts = []

    for ticker in sweeps.R3_AROON_CCI_INSTRUMENTS:
        ohlcv = load_ohlcv(ticker, TIMEFRAME)  # holdout excluded by default
        for family in sweeps.R3_AROON_CCI_FAMILIES:
            variants = sweeps.r3_aroon_cci_variants(family)
            ...
            run_path = ledger.run_and_record(
                ...
                notes=(f"Round 3 aroon-cci sweep (post-holdout DEV-ONLY, "
                       ...
                       f"r3-aroon-cci/{family}__{ticker}.json)"),
            )
            ...
            sweep_record = {
                ...
                "family_variants_tried": (grid_sizes[family]
                                          * len(sweeps.R3_AROON_CCI_INSTRUMENTS)),
