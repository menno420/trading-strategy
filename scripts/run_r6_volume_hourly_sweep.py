#!/usr/bin/env python3
"""Round 6 slice R6-C — volume families × 8 hourly tickers (lane r6-volume-hourly).

POST-HOLDOUT, DEV-ONLY (ORDER 014 item 2 night-run; pre-registered protocol:
``docs/research-round-6-plan.md`` § R6-C, merged BEFORE any Round-6 outcome
existed). Promotion is CLOSED: nothing this script produces can be an
out-of-sample claim; KEEPs are dev-candidates only.

Timeframe expansion of R6-A (the Round-3 slice-5/14/15 precedent): the SAME
two volume families — ``obv_trend`` and ``mfi_reversion`` — with the
IDENTICAL 12-variant grids (identity pinned by tests), on the committed
8-ticker hourly caches. Grid declared in ``trading_lab.sweeps`` BEFORE this
script ran (PR #116): 12 variants per family × 8 tickers = 192 registered
configs. Program burden prior to this slice: 4863 (4359 + R6-A 360 +
R6-B 144); program cumulative after: 5055. KNOWN weakness recorded in the
plan: the short dev-rail hourly history gives ~5 walk-forward splits per
lane — R5-B demoted the program's only prior hourly KEEP for exactly this
single-window-luck exposure, so any hourly KEEP here inherits that caveat.
...
full-period bookkeeping rows, the 1008/252-BAR walk-forward (lab
convention on hourly bars too) graded by the Round-2 rule +
``classify_verdict`` at K=12, the selection-fair standing gate ([D-0002])
on EVERY lane (its block = the R5-D fixed-config row), and one ledger run
for the top full-period variant (``variants_tried=12``). A cache too short
for one 1008/252 split records the honest first-class INSUFFICIENT-DATA
verdict (r3-trend-hourly precedent) — the method is never bent.

Runtime cap (pre-registered): <= 1200 s wall-clock for the whole slice,
checked before each lane; overrun STOPS the slice and records the partial
state + skipped lanes — no silent truncation.

Usage: python3 scripts/run_r6_volume_hourly_sweep.py
"""
...
def run_lane(ticker: str, family: str, grid_sizes: dict, lane_configs: int,
    ...
    variants = sweeps.r6_volume_hourly_variants(family)
    ...
    if len(ohlcv) < TRAIN_SIZE + TEST_SIZE:
        record = {
            ...
            "order": "ORDER 014 item 2 night-run (control/inbox.md); "
                     "pre-registered docs/research-round-6-plan.md § R6-C",
    ...
    run_path = ledger.run_and_record(
        ...
        notes=(f"Round 6 slice R6-C volume-hourly sweep (post-holdout "
    ...
    sweep_record = {
        ...
        "family_variants_tried": (grid_sizes[family]
                                  * len(sweeps.R6_VOLUME_HOURLY_INSTRUMENTS)),
...
def main() -> None:
    ...
    grid_sizes = sweeps.r6_volume_hourly_variants_per_family()
    lane_configs = sweeps.r6_volume_hourly_total_configs()
    program_cumulative = PROGRAM_PRIOR_CONFIGS + lane_configs
    assert grid_sizes == {"obv_trend": 12, "mfi_reversion": 12} \
        and lane_configs == 192, \
        "grid drifted from the declared Round-6 R6-C slice (trading_lab.sweeps)"
    for fam in sweeps.R6_VOLUME_HOURLY_FAMILIES:  # grid identity with R6-A
        assert (sweeps.r6_volume_hourly_variants(fam)
                == sweeps.r6_volume_variants(fam)), \
            f"R6-C grid for {fam} drifted from the R6-A grid (plan identity)"
    print(f"grids: {grid_sizes} × {len(sweeps.R6_VOLUME_HOURLY_INSTRUMENTS)} "
          f"instruments = {lane_configs} registered configs "
          f"(program cumulative {program_cumulative}); cap {CAP_SECONDS} s")

    lanes = [(t, f) for t in sweeps.R6_VOLUME_HOURLY_INSTRUMENTS
             for f in sweeps.R6_VOLUME_HOURLY_FAMILIES]
