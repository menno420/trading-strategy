#!/usr/bin/env python3
"""Round 7C slice R7-C — washout_recovery × 15 daily tickers (lane r7c-conjunction).

POST-HOLDOUT, DEV-ONLY (ORDER 018 owner overnight research GO; pre-registered
protocol: ``docs/research-round-7c-plan.md`` § R7-C, merged BEFORE any Round-7C
...
Family: ``washout_recovery`` (long iff SIMULTANEOUSLY washed out from a
long-horizon W_dd peak AND reclaiming a short-horizon W_prox high, two DISTINCT
windows, no hysteresis) — the CONJUNCTION of R7-A ``drawdown_reversion`` and
R7-B ``high_proximity`` on a decision surface neither evaluates. Grid declared
in ``trading_lab.sweeps`` BEFORE this script ran: 12 variants × 15 tickers =
180 registered configs. Program burden prior to this slice: 5415
(docs/research-round-7-results.md closing tally); program cumulative after this
R7-C slice: 5595.
...
Usage: python3 scripts/run_r7c_conjunction_sweep.py
"""
...
FAMILIES = ("washout_recovery",)

SWEEP_DIR = config.EXPERIMENTS_DIR / "sweeps" / SLICE

# Program burden prior to this slice: 5415 registered configs
# (docs/research-round-7-results.md closing tally). R7-C is the sole
# Round-7C slice, so its 180 land on top of 5415.
PROGRAM_PRIOR_CONFIGS = 5415
...
def run_lane(ticker: str, family: str, grid_sizes: dict, lane_configs: int,
    ...
    variants = sweeps.r7c_conjunction_variants(family)
    ...
    run_path = ledger.run_and_record(
        ...
        notes=(f"Round 7C slice R7-C washout_recovery sweep (post-holdout "
               f"DEV-ONLY, promotion closed — ORDER 018 owner overnight "
               f"research GO; pre-registered docs/research-round-7c-plan.md): "
    ...
    sweep_record = {
        ...
        "order": "ORDER 018 owner overnight research GO (control/inbox.md); "
                 "pre-registered docs/research-round-7c-plan.md § R7-C",
        ...
        "selection_gate": {
            "note": ("standing gate [D-0002] (docs/selection-fair-gate.md), "
                     "run on EVERY Round-7C lane; doubles as the R5-D "
...
def main() -> None:
    ...
    grid_sizes = {f: len(sweeps.r7c_conjunction_variants(f)) for f in FAMILIES}
    lane_configs = sum(grid_sizes.values()) * len(sweeps.R7C_INSTRUMENTS)
    program_cumulative = PROGRAM_PRIOR_CONFIGS + lane_configs
    assert grid_sizes == {"washout_recovery": 12} \
        and lane_configs == 180, \
        "grid drifted from the declared Round-7C slice"
    print(f"grids: {grid_sizes} × {len(sweeps.R7C_INSTRUMENTS)} "
          f"instruments = {lane_configs} registered configs "
          f"(program cumulative {program_cumulative}); cap {CAP_SECONDS} s")

    lanes = [(t, f) for t in sweeps.R7C_INSTRUMENTS for f in FAMILIES]
