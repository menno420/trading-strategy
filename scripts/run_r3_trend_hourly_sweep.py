#!/usr/bin/env python3
"""Round 3 slice 14 — trend/momentum families × all 8 tickers × HOURLY.
...
slice 5 (``scripts/run_r3_meanrev_hourly_sweep.py``, PR #85): NO new
strategy code — the four Round-3 trend/momentum single-instrument families
(``roc_momentum``, ``adx_filtered_sma``, ``aroon_trend``,
``trix_momentum``) are re-swept on hourly bars. Conventions follow the two
prior hourly lanes (``scripts/run_p1_trend_hourly_sweep.py`` /
docs/p1-trend-hourly-results.md and the slice-5 script) exactly:
...
* **Grids:** bar-denominated lookbacks reused as-is — on hourly bars the
  same numbers mean hours-to-days instead of days-to-weeks (roc lookback
  252 bars is ~38.8 hourly sessions ≈ 2 months instead of a year). Each
  family's declared Round-3 12-variant daily grid is reused VERBATIM by
  delegation in ``trading_lab.sweeps`` (declared BEFORE this script ran):
  12 variants per family × 8 tickers = 384 registered configs. Program
  burden prior to this lane: 3380 (through slice 13, PR #93); program
  cumulative after: 3764.
* **Walk-forward:** the lab's 1008/252/252 BAR convention, kept
  ...
  p1-trend-hourly and slice-5 lanes): ~7.4-month train windows,
...
* **Costs:** engine defaults, 5 bps slippage + 1 bp commission per side —
  at 1638 hourly bars/year this bites harder than on daily bars; trend
  families trade less than reversion, so part of what this lane measures
  is whether lower turnover preserves more of the gross edge intraday.
...
2. **A walk-forward evaluation** of the grid — the stitched OOS result is
   ...
   (``negative_bar_crossed``, as slice 13 recorded it): significantly
...
Usage: python3 scripts/run_r3_trend_hourly_sweep.py
"""
...
SWEEP_DIR = config.EXPERIMENTS_DIR / "sweeps" / "r3-trend-hourly"

# Program burden prior to this lane: 3380 = 3260 through slice 12
# (r3-gated-new-tickers, PR #92) + 120 slice 13 (r3-btc-coverage, PR #93
# session card).
PROGRAM_PRIOR_CONFIGS = 3380
...
def main() -> None:
    ...
    grid_sizes = sweeps.r3_trend_hourly_variants_per_family()
    lane_configs = sweeps.r3_trend_hourly_total_configs()
    program_cumulative = PROGRAM_PRIOR_CONFIGS + lane_configs
    assert grid_sizes == {fam: 12 for fam in sweeps.R3_TREND_HOURLY_FAMILIES} \
        and lane_configs == 384, \
        "grid drifted from the declared Round-3 slice-14 lane (trading_lab.sweeps)"
    print(f"grids: {grid_sizes} × {len(sweeps.R3_TREND_HOURLY_INSTRUMENTS)} "
    ...
    for ticker in sweeps.R3_TREND_HOURLY_INSTRUMENTS:
        ...
        for family in sweeps.R3_TREND_HOURLY_FAMILIES:
            variants = sweeps.r3_trend_hourly_variants(family)
            strategy_fn = STRATEGIES[family]
            base_record = {
                "schema_version": 1,
                "sweep": "r3-trend-hourly",
                ...
                "family_variants_tried": (
                    grid_sizes[family]
                    * len(sweeps.R3_TREND_HOURLY_INSTRUMENTS)),
            ...
            run_path = ledger.run_and_record(
                ...
                variants_tried=len(variants),
                notes=(f"Round 3 trend-hourly sweep (post-holdout "
                       f"DEV-ONLY, promotion closed — ORDER 012 night-run, "
                       f"slice 14, extends R3 after the slices-1-8 "
                       ...
                       f"experiments/sweeps/r3-trend-hourly/"
