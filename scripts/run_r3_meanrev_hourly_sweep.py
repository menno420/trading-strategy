#!/usr/bin/env python3
"""Round 3 slice 5 — mean-reversion families × all 8 tickers × HOURLY.
...
four existing mean-reversion families (``rsi_mean_reversion``,
``bollinger_reversion``, ``stochastic_reversion``,
``williams_r_reversion``) are re-swept on hourly bars. Conventions follow
the one prior hourly lane (``scripts/run_p1_trend_hourly_sweep.py`` /
docs/p1-trend-hourly-results.md) exactly:
...
* **Grids:** bar-denominated lookbacks reused as-is — on hourly bars the
  same numbers mean hours-to-days instead of days-to-weeks. Declared in
  ``trading_lab.sweeps`` BEFORE this script ran: 12 variants per family ×
  8 tickers = 384 registered configs. Program burden prior to this lane:
  1460 (680 pre-Round-3 + 192 r3-stoch-willr + 192 r3-roc-adx + 108
  r3-new-tickers + 288 r3-aroon-cci); program cumulative after: 1844.
* **Walk-forward:** the lab's 1008/252/252 BAR convention, kept
  ...
  p1-trend-hourly lane): ~7.4-month train windows, ~7.4-week test windows;
  on ~2476 bars it yields 5 splits and a stitched OOS of ~1260 bars
  ≈ 8.5 months — a single regime, and honestly labeled as such.
* **Costs:** engine defaults, 5 bps slippage + 1 bp commission per side —
  at 1638 hourly bars/year this bites much harder than on daily bars for
  high-turnover reversion; that is part of what this lane measures.
...
2. **A walk-forward evaluation** of the grid — the stitched OOS result is
   ...
   only, promotion closed. If a ticker's cache cannot support even one
   1008/252 split, the lane is recorded as a first-class
   ``INSUFFICIENT-DATA`` verdict row instead of bending the method.
3. **One standard ledger run file** for the family's top full-period
   variant, so the lane shows up in ``experiments/index.jsonl``.

Usage: python3 scripts/run_r3_meanrev_hourly_sweep.py
"""
...
SWEEP_DIR = config.EXPERIMENTS_DIR / "sweeps" / "r3-meanrev-hourly"

# Program burden prior to this lane: 680 pre-Round-3 (r3-stoch-willr
# script's ledger) + 192 r3-stoch-willr + 192 r3-roc-adx + 108
# r3-new-tickers + 288 r3-aroon-cci.
PROGRAM_PRIOR_CONFIGS = 1460
...
def main() -> None:
    ...
    grid_sizes = sweeps.r3_meanrev_hourly_variants_per_family()
    lane_configs = sweeps.r3_meanrev_hourly_total_configs()
    ...
    assert grid_sizes == {"rsi_mean_reversion": 12,
                          ...
                          "williams_r_reversion": 12} and lane_configs == 384, \
        "grid drifted from the declared Round-3 slice-5 lane (trading_lab.sweeps)"
    print(f"grids: {grid_sizes} × {len(sweeps.R3_MEANREV_HOURLY_INSTRUMENTS)} "
    ...
    for ticker in sweeps.R3_MEANREV_HOURLY_INSTRUMENTS:
        ...
        for family in sweeps.R3_MEANREV_HOURLY_FAMILIES:
            variants = sweeps.r3_meanrev_hourly_variants(family)
            strategy_fn = STRATEGIES[family]
            base_record = {
                "schema_version": 1,
                "sweep": "r3-meanrev-hourly",
                ...
                "family_variants_tried": (
                    grid_sizes[family]
                    * len(sweeps.R3_MEANREV_HOURLY_INSTRUMENTS)),
            ...
            run_path = ledger.run_and_record(
                ...
                variants_tried=len(variants),
                notes=(f"Round 3 meanrev-hourly sweep (post-holdout "
                       f"DEV-ONLY, promotion closed — ORDER 012 night-run): "
                       f"top full-dev-period variant of {len(variants)} "
                       f"tried (post-hoc selection — in-sample; the "
                       f"walk-forward OOS number lives in experiments/"
                       f"sweeps/r3-meanrev-hourly/{family}__{ticker}.json)"),
            )
