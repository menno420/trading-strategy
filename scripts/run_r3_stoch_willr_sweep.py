#!/usr/bin/env python3
"""Round 3 — stochastic + Williams %R reversion × all 8 tickers × daily.

POST-HOLDOUT, DEV-ONLY (ORDER 012 night-run: "expand the backtest surface —
new strategies, new indicators — every result recorded honestly"). Promotion
is closed: nothing this script produces can be an out-of-sample claim; KEEPs
are dev-candidates only.

Families: ``stochastic_reversion`` (slow %K, d_period frozen at 3) and
``williams_r_reversion`` — long/flat oversold cross-up reversion. Grid
declared in ``trading_lab.sweeps`` BEFORE this script ran: 12 variants per
family × 8 tickers = 192 registered configs. Program burden prior to this
lane: 680 (590 pre-Round-2 + 78 Round 2 + 12 MTF Bollinger grid); program
cumulative after: 872.
...
For every family × ticker this script produces (same shape as Round 2):
...
Usage: python3 scripts/run_r3_stoch_willr_sweep.py
"""
...
SWEEP_DIR = config.EXPERIMENTS_DIR / "sweeps" / "r3-stoch-willr"

# Program burden prior to this lane: 590 pre-Round-2 (pre-reg §2) + 78
# Round 2 (docs/research-round-2-results.md budget ledger) + 12 MTF
# Bollinger grid (docs/research/bollinger-mtf-dev-2026-07-12.md).
PROGRAM_PRIOR_CONFIGS = 680
...
def main() -> None:
    ...
    grid_sizes = sweeps.r3_stoch_willr_variants_per_family()
    lane_configs = sweeps.r3_stoch_willr_total_configs()
    program_cumulative = PROGRAM_PRIOR_CONFIGS + lane_configs
    assert grid_sizes == {"stochastic_reversion": 12,
                          "williams_r_reversion": 12} and lane_configs == 192, \
        "grid drifted from the declared Round-3 lane (trading_lab.sweeps)"
    print(f"grids: {grid_sizes} × {len(sweeps.R3_STOCH_WILLR_INSTRUMENTS)} "
          f"instruments = {lane_configs} registered configs "
          f"(program cumulative {program_cumulative})")
    verdicts = []

    for ticker in sweeps.R3_STOCH_WILLR_INSTRUMENTS:
        ohlcv = load_ohlcv(ticker, TIMEFRAME)  # holdout excluded by default
        for family in sweeps.R3_STOCH_WILLR_FAMILIES:
            variants = sweeps.r3_stoch_willr_variants(family)
            ...
            run_path = ledger.run_and_record(
                ...
                notes=(f"Round 3 stoch-willr sweep (post-holdout DEV-ONLY, "
                       ...
                       f"r3-stoch-willr/{family}__{ticker}.json)"),
            )
            ...
            sweep_record = {
                ...
                "family_variants_tried": (grid_sizes[family]
                                          * len(sweeps.R3_STOCH_WILLR_INSTRUMENTS)),
